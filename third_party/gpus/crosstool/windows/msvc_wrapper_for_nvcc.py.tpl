...
def InvokeNvcc(argv, log=False):
    ...
    # nvcc on -g option for MSVC generates /Zi, but we need /Z7 or nothing because bazel rules do not
    # know about vc140.pdb
    opt = []
    ...
    fatbin_options = ['--fatbin-options=' +
                      option for option in fatbin_options]
    ...
    for capability in compute_capabilities:
        capability = capability[len('sm_'):]
        nvccopts += [
            r'-gencode=arch=compute_%s,"code=compute_%s"' % (
